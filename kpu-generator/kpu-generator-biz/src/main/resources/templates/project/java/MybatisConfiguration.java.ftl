...
import cn.lmx.kpu.config.datasource.MybatisAutoConfiguration;
...
<#else>
    public class ${serviceNameUpper}MybatisConfiguration extends MybatisAutoConfiguration {
</#if>
...
@Override
protected List
<InnerInterceptor> getPaginationBeforeInnerInterceptor() {
    List
    <InnerInterceptor> list = new ArrayList<>();
