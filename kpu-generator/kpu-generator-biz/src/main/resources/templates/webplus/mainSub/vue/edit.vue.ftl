...
const ${sub.table.entityName?uncap_first}Ref = ref<typeof ${sub.table.entityName}>()
...
onMounted(async () => {
  ...
  if (data.value.form.id !== '') {
    getInfo()
    get${sub.table.entityName}Ref()?.load(props.type, data.value.form.id)
  }
})
            ...
            Object.values(errMap).forEach((errList: any) => {
              errList.forEach((params: any) => {
                ...
                rules.forEach((rule: any) => {
                  msgStr += `第 ${r"${rowIndex + 1}"} 行 ${r"${column.title}"} 校验错误：${r"${rule.message}"} <br/>`
                })
              })
            })
