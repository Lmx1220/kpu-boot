<template>
  <div class="bg-white m-4 mr-2 overflow-hidden">
    <div v-if="query" class="m-4">
      ...
      <Checkbox v-model:checked="recursion" @change="handleQuery">{{ t('title.tree_.includeSubNodes') }}</Checkbox>
    </div>
...
<script lang="ts">
  ...
  import { tree, remove } from '/~/${table.plusApplicationName}/api/${table.plusModuleName}/${table.entityName?uncap_first}'
