...
public class UriSecurityPreAuthAspect implements ApplicationContextAware {
     ...
     */
    @Around("execution(public * cn.lmx.basic.base.controller.*.*(..)) || " +
            "@annotation(cn.lmx.basic.annotation.security.PreAuth) || " +
            "@within(cn.lmx.basic.annotation.security.PreAuth)"
    )
    ...
    private void handleAuth(ProceedingJoinPoint point) {
        ...
        Boolean property = env.getProperty("kpu.security.enabled", Boolean.class, false);
        if (!property) {
